...
- **Data env:** `~/onyx/` — Nix flake + direnv with Python 3.13, snowflake-connector-python, pandas, matplotlib, seaborn, scipy. Use `cd ~/onyx && direnv exec . python3 script.py` or `cd ~/onyx && direnv exec . <command>`. Also has `snow sql` for quick queries.
